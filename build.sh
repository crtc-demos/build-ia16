...
if in_list binutils BUILDLIST; then
  ...
  ../binutils-ia16/configure --target=i386-unknown-elf --prefix="$PREFIX" 2>&1 | tee build.log
...
if in_list binutils-windows BUILDLIST; then
  ...
  ../binutils-ia16/configure --host=i686-w64-mingw32 --target=i386-unknown-elf --prefix="$PREFIX" | tee build.log
